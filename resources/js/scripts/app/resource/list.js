  ...
  if (dtInvoiceTable.length) {
    var dtInvoice = dtInvoiceTable.DataTable({
      ...
      columnDefs: [ 
        {
          ...
          orderable: false,
              ...
              var action = '<div class="d-flex align-items-center col-actions">';
                ...
                if (full['action']['edit']) {
                    action += '<a class="me-1" href="'+assetPath + 'admin/resource/'+full['slug']+'/edit'+'" title="Edit resource">' +
                  feather.icons['edit'].toSvg({ class: 'font-medium-2 text-body' }) +
                  '</a>' ;
                }              
                if (full['action']['delete']) {
                    action +='<a class=" delete-resource" data-id="' + full['slug']+
      ...
      drawCallback: function () {
        ...
        const deleteStockyard = document.querySelector('.delete-resource');
  ...
  // Suspend User javascript
        if (deleteStockyard) {
          $('.delete-resource').on('click', function () {
            let id = $(this).data('id');
            Swal.fire({
              title: 'Are you sure to delete resource?',
