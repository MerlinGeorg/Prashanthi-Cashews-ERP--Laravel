  ...
  if (dtInvoiceTable.length) {
    var dtInvoice = dtInvoiceTable.DataTable({
      ...
      columns: [
        ...
        { data: 'account_short_name', name: 'account_short_name' },
        { data: '' }
      ],
      columnDefs: [ 
        {
          ...
          render: function (data, type, full, meta) {
              ...
              if (full['action']['view']) {
                  action += '<a class="me-1" href="' +
                      assetPath + 'admin/account/' + full['slug'] +
                      '" title="View Account">' +
                      feather.icons['eye'].toSvg({ class: 'font-medium-2 text-body' }) +
                      '</a>';
              }
              if (full['action']['edit']) {
                  action += '<a class="" href="' + assetPath + 'admin/account/' + full['slug'] + '/edit' + '" title="Edit Account">' +
                      feather.icons['edit'].toSvg({ class: 'font-medium-2 text-body' }) +
                      '</a>';
              }              
              if (full['action']['delete']) {
                  action +='<a class="ms-1 delete-account" data-slug="' + full['slug'] +
